...
class AStarPlanner:
    ...
    def planning_1(self, sx, sy, p1x, p1y):
        """
        A star path search
        ...
        goal_node = self.Node(self.calc_xy_index(p1x, self.min_x), # calculate the index based on given position
                              self.calc_xy_index(p1y, self.min_y), 0.0, -1)
    ...
    def planning_2(self, p1x, p1y, p2x, p2y):
        """
        A star path search
        ...
        start_node = self.Node(self.calc_xy_index(p1x, self.min_x), # calculate the index based on given position
                               self.calc_xy_index(p1y, self.min_y), 0.0, -1) # set cost zero, set parent index -1
        goal_node = self.Node(self.calc_xy_index(p2x, self.min_x), # calculate the index based on given position
                              self.calc_xy_index(p2y, self.min_y), 0.0, -1)
    ...
    def planning_3(self, p2x, p2y, gx, gy):
        """
        A star path search
        ...
        start_node = self.Node(self.calc_xy_index(p2x, self.min_x), # calculate the index based on given position
                               self.calc_xy_index(p2y, self.min_y), 0.0, -1) # set cost zero, set parent index -1
...
def main():
    ...
    for i in range(20, 40): # draw the free border
        ox.append(i)
        oy.append(30.0)
    ...
    # set cost intesive area 1 the red one
    tc_x, tc_y = [], []
    ...
    # set cost intesive area 2 the yellow one
    fc_x, fc_y = [], []
    ...
    rx_1, ry_1 = a_star.planning_1(sx, sy, p1x, p1y)
    rx_2, ry_2 = a_star.planning_2(p1x, p1y, p2x, p2y)
    rx_3, ry_3 = a_star.planning_3(p2x, p2y, gx, gy)

    if show_animation:  # pragma: no cover
        plt.plot(rx_1, ry_1, "-r") # show the route 
        plt.plot(rx_2, ry_2, "-r") # show the route 
        plt.plot(rx_3, ry_3, "-r") # show the route 
