...
def main():
    ...
    def generate_points(min_distance):
        while True:
            sx, sy = random.randint(-10, 59), random.randint(-10, 59)
            gx, gy = random.randint(-10, 59), random.randint(-10, 59)
            if math.sqrt((sx - gx) ** 2 + (sy - gy) ** 2) >= min_distance:
                return sx, sy, gx, gy
    min_distance = 40
    sx, sy, gx, gy = generate_points(min_distance)
    grid_size = 1  # [m]
    robot_radius = 0.5 # [m]
    density = 0.3
    grid = [[0 for _ in range(70)] for _ in range(70)]
    buffer = 5
    ...
    def generate_obstacles(grid, density):
        num_obstacles = int(70 * 70 * density)
        for _ in range(num_obstacles):
            while True:
                x, y = random.randint(-9, 59), random.randint(-9, 59)
                if grid[y][x] == 0:  # 确保该位置没有障碍物
                    if not ((abs(x - sx) <= buffer and abs(y - sy) <= buffer) or (abs(x - gx) <= buffer and abs(y - gy) <= buffer)):
                        grid[y][x] = 1
                        break 
                    
    generate_obstacles(grid, density)
    ...
    def random_point():
        redx = random.randint(-10, 20)
        redy = random.randint(-10, 20)
        return redx, redy
    
    # set cost intesive area 2 the red one
    fc_x, fc_y = [], []
    redx, redy = random_point()
    for i in range(redx, redx+40):
        for j in range(redy, redy+40):
            fc_x.append(i)
            fc_y.append(j)
    ...
    if show_animation:  # pragma: no cover
        plt.plot(fc_x, fc_y, "oy") # plot the cost intensive area 2
